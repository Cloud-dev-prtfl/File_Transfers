 ...
 * V2 Updates: Intent Classification, CoT Reasoning, Dynamic Filters, Anti-Copycat, Top 5 Thresholding.
 */
...
function (serverWidget, llm, search, query) {
    ...
    const retrieveRelevantSearches = (userQueryVector) => {
        ...
        // Change 3: Expand Retrieval to Top 5 & Add a 0.70 Similarity Threshold
            ...
            async function sendQuery() {
                ...
                try {
                    ...
                    if (data.success) {
                        ...
                        if (data.error.includes("specialized NetSuite AI") || data.error.includes("related to creating")) {
                            errorPrefix = '🤖 Notice: ';
                        }
    ...
    const onRequest = (context) => {
        ...
        if (context.request.method === 'GET') {
            ...
            try {
                ...
                // --- Change 1: Pre-Search Intent Classification ---
                try {
                    const intentPrompt = `Analyze the following user input: "${userQuery}". Categorize it into exactly ONE of these tags: [GREETING], [OUT_OF_DOMAIN], [EXACT_MATCH], [CUSTOM_BUILD]. Respond ONLY with the tag.`;
                    const intentResponse = llm.generateText({
                        prompt: intentPrompt,
                        modelFamily: llm.ModelFamily.COHERE_COMMAND,
                        modelParameters: { temperature: 0.1, maxTokens: 20 }
                    });
                    const intentTag = intentResponse.text.toUpperCase();
                    
                    if (intentTag.includes('[GREETING]') || intentTag.includes('[OUT_OF_DOMAIN]')) {
                    ...
                    // Silently fail classification and proceed if the LLM hiccups
                }
                ...
                // --- Changes 2, 4 & 5: Anti-Copycat, CoT Reasoning, Dynamic Filters ---
                let currentPrompt = `You are a strict NetSuite SuiteScript 2.x expert bot. Analyze the request: "${userQuery}". 
You are provided with similar existing saved searches as context documents. DO NOT copy them exactly. Use them ONLY as a structural guide to understand NetSuite syntax. You MUST adapt the 'filters', 'columns', and 'type' to specifically satisfy the user's unique request.
Cheat Sheet for Filters: "Open Orders" = ['status', 'anyof', 'SalesOrd:A', 'SalesOrd:B', 'SalesOrd:D', 'SalesOrd:E']; "Main Line" = ['mainline', 'is', 'T']; "Active" = ['isinactive', 'is', 'F'].
First, briefly explain the changes you need to make or the logic you will apply based on the user's request vs the provided templates. Then, provide the final JSON configuration enclosed entirely within \`\`\`json and \`\`\` blocks.
The JSON MUST strictly contain 'type' (a string internal id like 'salesorder', 'customer', etc.), 'filters' (a valid array of filter expressions/objects), 'columns' (an array of strings or column objects), and a 'title' (a concise, descriptive name based on the user's request). DO NOT include an 'id' in the JSON.${customFieldMappingText}`;
                ...
                while (validationAttempts < maxAttempts) {
                    ...
                    // Change 4 Extension: Extract JSON specifically from the CoT output
                    const jsonMatch = generatedText.match(/```json\n([\s\S]*?)\n```/) || generatedText.match(/```\n([\s\S]*?)\n```/);
                    if (jsonMatch && jsonMatch[1]) {
                        jsonString = jsonMatch[1].trim();
                    } else if (generatedText.indexOf('{') !== -1 && generatedText.lastIndexOf('}') !== -1) {
                        jsonString = generatedText.substring(generatedText.indexOf('{'), generatedText.lastIndexOf('}') + 1);
                    }
                    
                    try {
                        const parsedSearchConfig = JSON.parse(jsonString);
                        ...
                        validationAttempts++;
                        currentPrompt = `You generated JSON that resulted in this NetSuite compilation/save error: ${e.message}. Fix the JSON structure and valid search column/filter syntax. Provide ONLY the corrected JSON configuration enclosed entirely within \`\`\`json and \`\`\` blocks.`;
                    }
                }
